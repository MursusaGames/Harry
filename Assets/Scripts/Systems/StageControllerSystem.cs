...
public class StageControllerSystem : MonoBehaviour
{
    ...
    private void Awake()
    {
       ...
       // if (!PlayerPrefs.HasKey(Constant.STAGE))
       // {
       //     PlayerPrefs.SetInt(Constants.STAGE, 0);
        //}
      /*  matchData.tryCount = stages[currentStage].tryCount;
        if (matchData.level > 2)
        {
            matchData.level = 0;
        }
        if (matchData.kitchenLevel > 2)
        {
            matchData.kitchenLevel = 0;
        }
        if (matchData.seaLevel > 2)
        {
            matchData.seaLevel = 0;
        }
        if (matchData.desertLevel > 2)
        {
            matchData.desertLevel = 0;
        }
        if (matchData.bugwardsLevel > 2)
        {
            matchData.bugwardsLevel = 0;
        }
        if (matchData.hellLevel > 2)
        {
            matchData.hellLevel = 0;
        }*/
