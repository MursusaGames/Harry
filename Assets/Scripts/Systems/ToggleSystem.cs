...
public class ToggleSystem : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        _speedSlider.value = (PlayerPrefs.GetFloat(Constant.SPEED)/350F);
    }
    ...
    public void ChangeSpeedVolume()
    {
        float value = _speedSlider.GetComponent<Slider>().value;
        //SoundDesigner.SetVolumeAudioSource(SoundBaseType.Sound, value);
        PlayerPrefs.SetFloat(Constant.SPEED, value*350);
        if (value == 0)
        {
            //sound.color = Color.red;
            //soundCheckmark.Hide();
